...
public class DephomosPlanetGenerator extends PlanetGenerator{
    ...
    Block[] terrain = {Blocks.darksand, Blocks.darksand, Blocks.cryofluid, Blocks.carbonStone, Blocks.basalt, Blocks.cryofluid, Blocks.cryofluid, Blocks.carbonStone};
    ...
    public Color getColor(Vec3 position){
        ...
        //more obvious color
        if(block == Blocks.carbonStone) block = Blocks.cryofluid;
    ...
    Block getBlock(Vec3 position){
        ...
        if(ice < 0.6){
            if(result == Blocks.darksand || result == Blocks.cryofluid || result == Blocks.basalt){
    ...
    public void genTile(Vec3 position, TileGen tile){
        ...
        if(tile.floor == Blocks.darksand && rand.chance(0.01)){
    ...
    protected void generate(){
        ...
        if(temp > 0.7){
            ...
            pass((x, y) -> {
                if(floor != Blocks.basalt){
                    ...
                    if(noise > 0.55f && floor == Blocks.darksand){
        ...
        //arkycite
        pass((x, y) -> {
            if(floor != Blocks.darksand) return;
        ...
        blend(Blocks.cryofluid, Blocks.darksand, 4);

        //TODO may overwrite floor blocks under walls and look bad
        blend(Blocks.slag, Blocks.basalt, 4);
        ...
        pass((x, y) -> {
            //rough rhyolite
            if(noise(x, y + 600 + x, 5, 0.86f, 60f, 1f) < 0.41f && floor == Blocks.cryofluid){
                floor = Blocks.cryofluid;
            }

            if(floor == Blocks.cryofluid && Mathf.within(x, y, spawnX, spawnY, 30f + noise(x, y, 2, 0.8f, 9f, 15f))){
                floor = Blocks.darksand;
            }

            if((floor == Blocks.cryofluid || floor == Blocks.darksand) && block.isStatic()){
                block = Blocks.carbonWall;
            }
            ...
            if(max > 0){
                ...
                if(block == Blocks.air) block = Blocks.carbonStone;
            }

            if(floor == Blocks.darksand && noise(x + 78 + y, y, 3, 0.8f, 6f, 1f) > 0.44f){
                floor = Blocks.cryofluid;
            }

            if(floor == Blocks.darksand && noise(x + 78 - y, y, 4, 0.73f, 19f, 1f) > 0.63f){
                floor = Blocks.carbonStone;
            }
        });
        ...
        blend(Blocks.darksand, Blocks.basalt, 4);
        ...
        for(Tile tile : tiles){
            ...
            if((floor == Blocks.darksand || floor == Blocks.cryofluid) && rand.chance(0.004)){
                int radius = 2;
                for(int x = -radius; x <= radius; x++){
                    for(int y = -radius; y <= radius; y++){
                        Tile other = tiles.get(x + tile.x, y + tile.y);
                        if(other == null || (other.floor() != Blocks.carbonStone && other.floor() != Blocks.roughRhyolite) || other.block().solid){
                            continue outer;
                        }
                    }
                }
                ...
                for(var pos : SteamVent.offsets){
                    ...
                    other.setFloor(Blocks.carbonVent.asFloor());
                }
            }
        }
        ...
        while(ventCount < minVents && iterations++ < maxIterations){
            ...
            for(Tile tile : tiles){
                if(rand.chance(0.00018 * (1 + iterations)) && !Mathf.within(tile.x, tile.y, spawnX, spawnY, 5f)){
                    ...
                    Block
                            floor = Blocks.carbonStone,
                            secondFloor = Blocks.carbonBoulder,
